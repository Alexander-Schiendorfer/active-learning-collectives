...
c3_economically_acc_b:((production["b"][t] >= 18 && production["b"][t] <= 33) && penalties["c3_economically_acc_b"][t] == 0) || (!(production["b"][t] >= 18 && production["b"][t] <= 33) &&  penalties["c3_economically_acc_b"][t]== 1);
	...
	forall (t in 0..LAST_SIMULATION_STEP-1) {	
		fixed_change_b: (running["b"][t] == 1 && running["b"][t+1] == 1) => abs(production["b"][t] - production["b"][t+1]) <= fixedChange_b;
		minimal_up_time_b: (running["b"][t] == true && running["b"][t+1] == false) => (consRunning["b"][t] - minUpTime_b) >= 0;
		switch_off_min_b: (running["b"][t] == true && running["b"][t+1] == false) => production["b"][t] == P_min_b;
...
c2_economically_good_c:((production["c"][t] >= 280.0 && production["c"][t] <= 370.0) && penalties["c2_economically_good_c"][t] == 0) || (!(production["c"][t] >= 280.0 && production["c"][t] <= 370.0) &&  penalties["c2_economically_good_c"][t]== 1);
	    signal_on_idle_c: signals[t] == 1 => powerPlantState[t] == IDLE_c;
   		signal_off_idle_c: signals[t] == -1 => powerPlantState[t] == ON_c;
	}   	
...
c3_rate_of_change_opt_c:(((running["c"][t] == 1) => abs(production["c"][t] - production["c"][t+1]) <= production["c"][t]  * 0.1) && penalties["c3_rate_of_change_opt_c"][t] == 0) || (!((running["c"][t] == 1) => abs(production["c"][t] - production["c"][t+1]) <= production["c"][t]  * 0.1) &&  penalties["c3_rate_of_change_opt_c"][t]== 1);
        ...
        state_running_c: (powerPlantRunning[t]) == (powerPlantState[t] == ON_c);
	}
	
	forall ( t in 0..card(TIMERANGE)-2 ) {
	  	powerPlantState[t] == IDLE_c => (powerPlantState[t+1] == IDLE_c || (powerPlantState[t+1] == STARTING_c && signal[p][t] == 1 && (countdown[t+1] == startupCountDown_c)));
	    (powerPlantState[t] == STARTING_c && countdown[t] >= 1) => (powerPlantState[t+1] == STARTING_c && countdown[t+1] == countdown[t] - 1);
	    (powerPlantState[t] == STARTING_c && countdown[t] == 0) => powerPlantState[t+1] == ON_c;
	    powerPlantState[t] == ON_c => (powerPlantState[t+1] == ON_c || (powerPlantState[t+1] == IDLE_c && signal[t] == -1)); 
	}
